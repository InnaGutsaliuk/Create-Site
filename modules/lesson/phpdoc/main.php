...
class Registration {
    ...
    function sendActivate($id,$hash,$email) {
        try {
            ...
            $mail->msgHTML(\'<div>Вы успешно зарегистрировались на сайте.</div>
                <div>Для завершения регистрации пройдите по ссылке:<br>
                <a href="\'.Core::$DOMAIN.\'/login/activate/\'.$id.\'/\'.$hash.\'">\'.Core::$DOMAIN.\'/login/activate/\'.$id.\'/\'.$hash.\'</a>
                </div>\');
            $mail->send();
            unset($mail);
        } catch(\Exception $e) {
